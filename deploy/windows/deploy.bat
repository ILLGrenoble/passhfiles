...
for /F %%a in ('.\ci-install\python.exe -c "exec(open('.\src\passhfiles\__pkginfo__.py').read()) ; print(__version__)"') do set version=%%a
...
del /Q %target_dir%\passhfiles-*-win-amd64.exe
